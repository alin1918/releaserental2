...
class QtyBooked extends \Magento\Ui\Component\Listing\Columns\Column
{
    /**
     * Column name
     */
    const NAME = 'column.qty_booked';
        ...
        if (isset($dataSource['data']['items'])) {
            ...
            foreach ($dataSource['data']['items'] as & $item) {
                if ($this->helperRental->isRentalType($item['entity_id'])) {
                    $item[$fieldName] =
                        $this->stockManagement->getSirentQuantity($item['entity_id']) -
                        $this->stockManagement->getAvailableQuantity($item['entity_id'], $startDate, $endDate);
                }
            }
        }
