...
class Booked extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        {
            if(date("Y-m-d", strtotime($inventory[$i]['e'])) >= date("Y-m-d", strtotime("now")))
            {
                $inventory2[] = $inventory[$i];
            }
        }
        
        
        $inventoryFull = $this->stock->updateFullDatesBooking($inventory2);
