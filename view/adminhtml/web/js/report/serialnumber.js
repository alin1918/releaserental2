define([
    ...
    'text!SalesIgniter_Rental/template/report/serialnumber/product.html',
    'text!SalesIgniter_Rental/template/report/serialnumber/calendar/day.html',
    'text!SalesIgniter_Rental/template/report/serialnumber/calendar/week.html',
    'text!SalesIgniter_Rental/template/report/serialnumber/calendar/month.html',
    'salesigniter/report/item/product',
    'salesigniter/report/item/serialnumber'
], function (ko, $, moment, Calendar, Pager, Filter, ProductTemplate, DayTemplate, WeekTemplate, MonthTemplate, ProductFactory, SerialFactory) {
    ...
    function RentalReport() {
        ...
        this.getDateContent = function (Result, o) {
            if (o.item.constructor.name == 'SerialNumber') {
                this._getDateContentSerialNumber(Result, o.item, o.date);
            }
            else if (o.item.constructor.name == 'Product') {
        ...
        this._getDateContentSerialNumber = function (Result, SerialNumber, Date) {
            if (SerialNumber.getStatus()) {
                var Statuses = SerialNumber.getStatus() || [];
                if (typeof Statuses[Date.unix()] !== 'undefined') {
                    Result.html = Statuses[Date.unix()].result;
                }
            }
        };
        ...
        this.setReportData = function (ReportData) {
            for (var i = 0; i < ReportData.products.length; i++) {
                if (typeof ReportData.products[i].serial_numbers !== 'undefined') {
                    for (var n = 0; n < ReportData.products[i].serial_numbers.length; n++) {
                        ReportData.products[i].serial_numbers[n] = SerialFactory.create(ReportData.products[i].serial_numbers[n]);
                        ReportData.products[i].serial_numbers[n].setReport(this);
                    }
                }
        ...
        this.showDateReport = function (e, data, Item) {
            ...
            if (Item.constructor.name == 'Product') {
                ...
                dataUrl = self.getCalendar().getDateDataUrlProduct();
            }
            else if (Item.constructor.name == 'SerialNumber') {
                RequestParams.serial = Item.getNumber();
                dataUrl = self.getCalendar().getDateDataUrl();
                var ReservationOrderId = Item.getReservationOrderId(data.timestamp);
                if (ReservationOrderId) {
                    RequestParams.reservation_order_id = ReservationOrderId;
                }
            }
