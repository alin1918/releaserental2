...
console.log('update price box price: ' + price);
			if (typeof priceBox.priceBox('option').priceConfig !== 'undefined') {
                            console.log('updating price box');
				...
				if (price > 0) {
					$('.product-top-info-customizebox .price-final_price .product_top_price .price-container .price-wrapper').css('display', '');
                    $('.product-top-info-customizebox .price-final_price .product_top_price .price-container .price-wrapper').html(priceTemplate({data: finalPrice}));
...
//                        if (this.options.fixedRentalLength > 0) {
			...
			$.ajaxq('booked' + this.element.attr('id'), {
				...
				success: function (res) {
					...
					if (res.pricingPpr !== false) {
						...
						// if (pricePpr.length > 0) { // NVNCBL COMMENTED OUT
						if (pricePpr.length > 0 && res.pricingPpr != '' ) { // NVNCBL CUSTOM
		...
		_initPicker: function () {
			...
			this.options.selectorProduct = '.product-info-main, .product-add-form, .bundle-options-container';
