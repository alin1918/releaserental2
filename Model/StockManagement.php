	...
	public function reserveOrder(\Magento\Sales\Api\Data\OrderInterface $order)
	{
		...
		foreach ($order->getItems() as $item) {
			...
			if ($item->getProductType() === Sirent::TYPE_RENTAL) {
				...
				/* NVNCBL CUSTOM */
				// identify serials to ship
				$this->searchCriteriaBuilder->addFilter('product_id', $item->getProductId());
				$this->searchCriteriaBuilder->addFilter('status', 'available');

				$criteria = $this->searchCriteriaBuilder->create();
				$serials_shipped = array();
				$serials = $this->serialNumberDetailsRepository->getList($criteria)->getItems();
				foreach ( $serials as $serial ) {
					$serials_shipped[] = $serial->getSerialnumber();
					break;
				}
				/* END NVNCBL CUSTOM */
				...
				if (!$dates->getIsBuyout()) {
					$data = [
						...
						'serials_shipped' => implode( ',', $serials_shipped ), // NVNCBL CUSTOM
					...
					//$this->saveFromArray($data); // NVNCBL COMMENTED OUT

					/* BEGIN NVNCBL CUSTOM */
					// record these serials as booked
					$reservationOrder = $this->saveFromArray($data);
					if( $reservationOrder->getId() ){
						$this->serialNumberDetailsRepository->updateSerials($reservationOrder->getProductId(), 'booked', $serials_shipped, $reservationOrder->getId());
					}
					/* END NVNCBL CUSTOM */
