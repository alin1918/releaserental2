...
class Sirent extends \Magento\CatalogImportExport\Model\Import\Product\Type\AbstractType {
	 ...
	 * @param \SalesIgniter\Rental\Model\SerialNumberDetailsFactory                    $SerialNumberFactory
		...
		if ( $this->_entityModel->getBehavior() == \Magento\ImportExport\Model\Import::BEHAVIOR_DELETE ) {
			...
			while ( $bunch = $this->_entityModel->getNextBunch() ) {
				foreach ( $bunch as $rowNum => $rowData ) {
					$skuVal       = strtolower( $rowData[ \Magento\CatalogImportExport\Model\Import\Product::COL_SKU ] );
					$productData  = $newSku[ $skuVal ];
	...
	protected function insertSerials() {
		...
		$this->connection->insertOnDuplicate( $serialNumberTable, $insert, [
			'notes',
			'cost',
			'date_acquired',
			'status'
